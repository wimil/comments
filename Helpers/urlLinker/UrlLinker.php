    ...
    public function __construct(array $options = [])
    {
        ...
        foreach ($default_options as $key => $value) {
            ...
            switch ($key) {
                ...
                case 'allowEmailLinkCreator':
                    $this->allowEmailLinkCreator = (bool) $value;
                    break;
                ...
                case 'validDomainTDL':
                    $this->validDomainTDL = (array) $value;
                    break;
            }
        }
    }
        ...
        while (preg_match($this->buildRegex(), $text, $match, PREG_OFFSET_CAPTURE, $position)) {
            ...
            if (preg_match('{^\.[0-9]{1,3}$}', $tld) || isset($validTlds[$tld])) {
                ...
                if (!$scheme && $username && !$password && !$afterDomain) {
                    ...
                    $validDomainTDL = $this->validDomainTDL;
                    $linkText = "$domain$port$path";
                    $htmlLinkCreator = $this->htmlLinkCreator;
                    if ($scheme) {
                        $completeUrl = $url;
                        $link = $htmlLinkCreator($completeUrl, $linkText);
                    } else {
                        if (in_array($tld, $validDomainTDL)) {
